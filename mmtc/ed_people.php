...
// adhoc stuff, and it provides security/options so mucking with it is messy ????
	...
	require(in_parent_path("common/common_mysql.php"));
...
<?php
	...
	$schema_types = uncache_variable(in_parent_path("rpc/templates/journal_types.ser"));
	...
	if (isset($edAction)) {
		if ($edAction == "Create people") {
			...
			while (list($k,$v) = each($del)) {
				$query = "select * from people where code=$v";
				$result = mysql_query($query);
				if ($result != 0) {
					$nitems = mysql_num_rows($result);
					if ($nitems > 0) {
						$row = mysql_fetch_object($result);
						$xcode = $row->code;
						$xpropa = split(",",$row->properties);
						$mmk = index_of('mmtc', $xpropa);
						if ($mmk>=0) {
							unset($xpropa[$mmk]);
							$xprops = list_string($xpropa);
							$query = "update people set properties='$xprops' where code=$v";
							echo $query;
							$result = mysql_query($query);
							if (!$result) {
				...
				} else { // ignore the oops for a bit
				}
			}
...
//				echo "modify = $query<BR>";
			...
			if ($stnumber) {
				$query = "select * from people where stnumber='$stnumber'";
				$result = mysql_query($query);
				if ($result != 0) {
					$nitems = mysql_num_rows($result);
					if ($nitems > 0) {
						...
						$xcode = $row->code;
						$xpropa = split(",",$row->properties);
						if (index_of('mmtc', $xpropa)<0) {
							$xpropa[] = 'mmtc';
							$xprops = list_string($xpropa);
							$query = "update people set properties='$xprops' where stnumber='$stnumber'";
							$result = mysql_query($query);
							if (!$result) {
								errorpage("Update error ".mysql_error());
							}
						} else {
							header("Location: ed_people.php?selected=$selected&action_msg=$stnumber is already enabled for the MMTC ...");
							exit;
						}
						header("Location: ed_people.php?selected=$selected&action_msg=Enabled $stnumber for the MMTC ...");
						exit;
					}
				} else { // ignore the oops for a bit
				}
				$set = "";
				$set = set_item($set, "stnumber", strtolower($stnumber));
				$set = set_item($set, "surname", $surname);
				$set = set_item($set, "firstname", $firstname);
				$set = set_item($set, "title", $title);
				if (isset($passwd)) {
				...
				$set = set_item($set, "web", $web);
				$set = set_item($set, "email", $email);
				$set = set_item($set, "properties", "mmtc");
				$set = set_item($set, "description", $description);
				$where = "code = $key";
				if ($set) {
	...
	//				echo "insert = $query<BR>";
				...
				header("Location: ed_people.php?selected=$selected&action_msg=Users must have a staff/student number ...");
				exit;
			}
	...
	div("wide-margin");
?>
<p>This page operates on the same user database as the RPC, providing an interface for user administration
geared to the needs of the MMTC site.<br>
Inserting a user (identified by staff/student number)
already in the RPC user database will merely enable
that user for this database, and keep the associated already in the database.<br>
Deleting a user from this page will merely disable the MMTC privilege, but keep their details in the
RPC database (NB. deletion from the corresponding RPC user control page is permanent!)<br>
Some operations may require the use of the RPC user database interface.</p>
<?php
	if ($action_msg)
		echo $action_msg;
...
<?php
if ($login_type != LOGIN_USER):
//   echo "<TD WIDTH='15%' ALIGN='center' VALIGN='top'><B>Properties</B><BR>";
//   checkbox_array("property[]", $properties_values);
//   echo "</td>";
endif;
?>
 </TABLE>
...
//		null,
//		null);
	document.edPeopleForm.selected.value = <?php echo $selected?$selected:0; ?>;
	display_selected(document.edPeopleForm.selected.value);
</SCRIPT>
<?php
	endif;
	mysql_free_result($result);
